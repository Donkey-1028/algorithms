...
input_count, need_count = map(int, stdin.readline().split(' '))
lan = []
...
def lan_binary_search(array, count):
    ...
    while low <= high:
        ...
        for _, value in enumerate(array):
            result += value // mid
        if result >= count:
            max_length = mid  # 입력받은 수에 맞게 나눠진 최대 길이를 저장
            low = mid + 1
        elif result < count:
            high = mid - 1
...
print(lan_binary_search(lan, need_count))
