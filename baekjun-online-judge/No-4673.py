...
def find_create_number(n):
    create_number = [n]
    individual_number = list(str(n))
    create_number = create_number + individual_number
    result = 0
    for _, value in enumerate(create_number):
        result += int(value)

    return result


le = [find_create_number(n) for n in range(10000)]
le = set(le)
